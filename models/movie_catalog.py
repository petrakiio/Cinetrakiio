...
for genre_name, movies in MOVIES_BY_GENRE.items():
    ...
    for slug, template_name in movies.items():
        ...
        file_to_slug[normalize_legacy_token(filename)] = slug
        file_to_slug[normalize_legacy_token(slug)] = slug
    LEGACY_FILE_TO_SLUG_BY_GENRE[genre_name] = file_to_slug


def get_movie_template(genre_name: str, slug: str):
    return MOVIES_BY_GENRE.get(genre_name, {}).get(slug)


def resolve_canonical_genre(*raw_values: str):
    for raw_value in raw_values:
        if not raw_value:
            continue

        direct = LEGACY_GENRE_ALIASES.get(raw_value.lower())
        if direct:
            return direct

        normalized = LEGACY_GENRE_ALIASES.get(normalize_legacy_token(raw_value))
        if normalized:
            return normalized
...
def resolve_slug_from_legacy_file(genre_name: str, legacy_file: str):
    return LEGACY_FILE_TO_SLUG_BY_GENRE.get(genre_name, {}).get(normalize_legacy_token(legacy_file))
